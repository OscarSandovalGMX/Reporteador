...
function load_Data(Consulta, Tipo) {
    $.ajax({
        ...
        url: 'ConsultaBD.asmx/ObtieneDatos',
            ...
            if (response.d.length > 0) {
                ...
                for (var i = 0; i < response.d.length; i++) {
                        $("[id*=gvd_Catalogo]").append('<tr>' +
                                                       ...
                                                       '<td><label id="lbl_ClaveCat" style="Width:80px;font-size:8.5px;">' + response.d[i].Clave + '</label></td>' +
                                                       '<td><label id="lbl_DesCat" style="Width:245px;font-size:8.5px;">' + response.d[i].Descripcion + '</label></td>' +
                                                  '</tr>')                   
                };
...
function load_resultado(Consulta) {
    $.ajax({
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'ConsultaBD.asmx/ObtieneResultado',
        data: "{ 'Consulta': '" + Consulta + "'}",
        dataType: 'JSON',
        success: function (response) {
            if (response.d.length > 0) {
...
$("body").on("click", ".Seccion1", function () {
    event.preventDefault ? event.preventDefault() : event.returnValue = false;
    var cod_reporte = $("input[id$='hid_codReporte']")[0].value;
    $("input[id$='hid_Catalogo']")[0].value = "Fld1"
    $("#lblCatalogo")[0].innerText = 'Generales';
    var strSel = $("input[id$='hid_IdSeccion1']")[0].value;
    var Condicion = " WHERE cod_reporte IN (" + cod_reporte + ") AND cod_seccion IN (1) ";
    load_Data("spS_CatalogosOP ==Rep==,==" + Condicion + "==,==" + strSel + "==", "Multiple");
});

$("body").on("click", ".Seccion2", function () {
    event.preventDefault ? event.preventDefault() : event.returnValue = false;
    var cod_reporte = $("input[id$='hid_codReporte']")[0].value;
    $("input[id$='hid_Catalogo']")[0].value = "Fld2"
    $("#lblCatalogo")[0].innerText = 'Reaseguro';
    var strSel = $("input[id$='hid_IdSeccion2']")[0].value;
    var Condicion = " WHERE cod_reporte IN (" + cod_reporte + ") AND cod_seccion IN (2) ";
    load_Data("spS_CatalogosOP ==Rep==,==" + Condicion + "==,==" + strSel + "==", "Multiple");
});

$("body").on("click", ".Seccion3", function () {
    event.preventDefault ? event.preventDefault() : event.returnValue = false;
    var cod_reporte = $("input[id$='hid_codReporte']")[0].value;
    $("input[id$='hid_Catalogo']")[0].value = "Fld3"
    $("#lblCatalogo")[0].innerText = 'Siniestros';
    var strSel = $("input[id$='hid_IdSeccion3']")[0].value;
    var Condicion = " WHERE cod_reporte IN (" + cod_reporte + ") AND cod_seccion IN (3) ";
    load_Data("spS_CatalogosOP ==Rep==,==" + Condicion + "==,==" + strSel + "==", "Multiple");
});

$("body").on("click", ".Seccion4", function () {
    event.preventDefault ? event.preventDefault() : event.returnValue = false;
    var cod_reporte = $("input[id$='hid_codReporte']")[0].value;
    $("input[id$='hid_Catalogo']")[0].value = "Fld4"
    $("#lblCatalogo")[0].innerText = 'Cúmulos';
    var strSel = $("input[id$='hid_IdSeccion4']")[0].value;
    var Condicion = " WHERE cod_reporte IN (" + cod_reporte + ") AND cod_seccion IN (4) ";
    load_Data("spS_CatalogosOP ==Rep==,==" + Condicion + "==,==" + strSel + "==", "Multiple");
});

$("body").on("click", ".Seccion5", function () {
    event.preventDefault ? event.preventDefault() : event.returnValue = false;
    var cod_reporte = $("input[id$='hid_codReporte']")[0].value;
    $("input[id$='hid_Catalogo']")[0].value = "Fld5"
    $("#lblCatalogo")[0].innerText = 'Cobranzas';
    var strSel = $("input[id$='hid_IdSeccion5']")[0].value;
    var Condicion = " WHERE cod_reporte IN (" + cod_reporte + ") AND cod_seccion IN (5) ";
    load_Data("spS_CatalogosOP ==Rep==,==" + Condicion + "==,==" + strSel + "==", "Multiple");
});

$("body").on("click", "[id*=btn_ReplicaFiltro]", function () {
    event.preventDefault ? event.preventDefault() : event.returnValue = false;
    $("input[id$='hid_Todos']")[0].value = 0;
    OpenPopup('#ReplicaModal');
});

$("body").on("click", "[id*=btn_ReplicaConsulta]", function () {
    event.preventDefault ? event.preventDefault() : event.returnValue = false;
    $("input[id$='hid_Todos']")[0].value = 1;
    OpenPopup('#ReplicaModal');
});
...
$("body").on("click", "[id*=gvd_Versiones] .Temporal", function () {
    var row = $(this).closest("tr");
    var chk_Temporal = row.find('.Temporal');
    if ($(chk_Temporal)[0].firstChild.checked == false) {

        if (confirm('¿Esta seguro de deshabilitar el Ambiente Temporal?')) {
            __doPostBack($(chk_Temporal)[0].firstChild.name, '');
        } else {
            event.preventDefault ? event.preventDefault() : event.returnValue = false;
        }
    }
...
$("body").on("focus", "[id*=gvd_Formulas] .Formula", function () {
    var row = $(this).closest("tr");
    var Control = row.find('.Formula');
    Seleccion($(Control)[0]);
});
...
$("body").on("click", ".modalTabla", function () {
    // set ohters element to the initial level
    $(this).siblings('.modalTabla').css('z-index', 10);
